...
void ImageConverter::invertImage() {
	...
	Canvas canvas1(myWidth+50, 0, myWidth, myHeight, myImageFileName + " Inverted");
	canvas1.start();
...
void ImageConverter::flipHorizontal() {
	...
	Canvas canvas4(myWidth+50, 0, myWidth, myHeight, myImageFileName + " Inverted");
	canvas4.start();
	Background * background0 = myCanvas0.getBackground();
	Background * background4 = canvas4.getBackground();
...
#pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // bottom to top
		for (int x = START_X; x < STOP_X; ++x)  {      // left to right
			...
			background4->drawPixel(-x, y, color);
		} // inner for
		// slow the processing to simulate a very large image
		canvas4.sleep();
	} // outer for
...
void ImageConverter::flipVertical() {
	...
	Canvas canvas5(myWidth+50, 0, myWidth, myHeight, myImageFileName + " Inverted");
	canvas5.start();
	Background * background0 = myCanvas0.getBackground();
	Background * background4 = canvas5.getBackground();
