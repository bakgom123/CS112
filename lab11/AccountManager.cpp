...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if (it != myAccounts.end()){
		...
		throw Exception("changeUserName(const string& oldUserName, const string& newUserName)", newUserName + "dose not exist.");
	}
}
...
void AccountManager::deleteAccount(const string& userName) {
	...
	if (it != myAccounts.end()){
		...
		throw Exception ("createNewAccount(user, pass)", userName + "does not exist");
	}
}
...
void AccountManager::changePassword(const string& userName,const string& newPassword) {
	map<string, string>::iterator it = myAccounts.find(userName);
		if (it != myAccounts.end()){
			try {
				myAccounts.erase(it);
				createNewAccount (userName, newPassword);
			} catch (Exception& e){
				throw e;
			}
		} else {
			throw Exception ("AccountManager::changePassword(const string& userName, const string& newPassword)", userName + "does not exist");
		}
}
